...
export default function OnlineDiscoveryLayer() {
  ...
  return (
    <div className="space-y-0 px-2 md:px-4">
      {/* ── How to Find QD Suppliers on 1688 + YouTube ── */}
      <div className="py-6">
        <h4 className="text-xl md:text-2xl font-semibold text-wf-text mb-4">How to find Qingdao suppliers on 1688</h4>
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left: Steps */}
          <div className="space-y-4">
            {[
              { step: "1", title: "Search in Chinese", desc: "Use Chinese product names. Examples: 青岛轮胎 (Qingdao tires), 平度假睫毛 (Pingdu false eyelashes), 即墨服装批发 (Jimo clothing wholesale), 青岛海鲜 (Qingdao seafood). WorldFirst’s AI Sourcing Agent translates for you." },
              { step: "2", title: "Filter by Qingdao", desc: "Set the supplier location filter to 青岛 (Qingdao) or 山东 (Shandong) province. For eyelashes, filter to 平度 (Pingdu). For textiles, filter to 即墨 (Jimo)." },
              { step: "3", title: "Check factory badges", desc: "Look for 实力商家 (verified merchant) and 源头工厂 (source factory) tags. For tires, check if they list production capacity and certifications (DOT, ECE, CCC)." },
              { step: "4", title: "Verify before paying", desc: "Cross-check the business registration at gsxt.gov.cn. Pingdu addresses = eyelash/rubber factories. Huangdao addresses = heavy industry. Jimo addresses = textiles. Pay through 1688 World Pay for protection." },
            ].map((s) => (
