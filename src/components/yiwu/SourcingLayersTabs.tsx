...
const sections = [
  {
    id: "online",
    label: "Search on 1688, Yiwugo & Chinagoods",
    icon: (
  ...
  {
    id: "markets",
    label: "Wholesale markets in Yiwu",
    icon: (
  ...
  {
    id: "factories",
    label: "WorldFirst verified suppliers in Yiwu",
    icon: (
    ...
    <section id="find-suppliers" className="py-14 md:py-20 bg-wf-bg-light">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="text-3xl md:text-4xl font-semibold text-wf-text mb-3">
          How to Find <span className="accent-word">Suppliers</span> in Yiwu
        </h2>
        <p className="hidden md:block text-wf-text-secondary mb-10 max-w-2xl">
          Four ways to find Yiwu suppliers. Most buyers combine online discovery with market visits and agent support.
        </p>
          ...
          {sections.map((section) => (
            <div key={section.id} className="bg-white border border-wf-border rounded-2xl overflow-hidden">
              ...
              <div className="px-6 pb-6 pt-5">
                {/* ── Search on 1688, Yiwugo & Chinagoods ── */}
                {section.id === "online" && (
                  <div>
