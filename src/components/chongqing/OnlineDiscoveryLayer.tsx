...
export default function OnlineDiscoveryLayer() {
  ...
  return (
    <div className="space-y-0 px-2 md:px-4">
      {/* ── How to Find CQ Suppliers on 1688 + YouTube ── */}
      <div className="py-6">
        <h4 className="text-xl md:text-2xl font-semibold text-wf-text mb-4">How to find Chongqing suppliers on 1688</h4>
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left: Steps */}
          <div className="space-y-4">
            {[
              { step: "1", title: "Search in Chinese", desc: "Use Chinese product names. Examples: 重庆摩托车 (motorcycles), 重庆汽车配件 (auto parts), 火锅底料 重庆 (hotpot base), 重庆电脑配件 (computer accessories). WorldFirst's AI Sourcing Agent translates for you." },
              { step: "2", title: "Filter by Chongqing", desc: "Set the supplier location filter to 重庆 (Chongqing). Chongqing is a direct-controlled municipality — it's its own province-level entity, so the filter is straightforward." },
              { step: "3", title: "Check factory badges", desc: "Look for 实力商家 (verified merchant) and 源头工厂 (source factory) tags. For motorcycles and auto parts, verify the supplier has actual manufacturing capability — check for 生产 (production) in their 经营范围 (business scope)." },
              { step: "4", title: "Verify before paying", desc: "Cross-check the business registration at gsxt.gov.cn. For food products, confirm SC food production certification. Pay through 1688 World Pay for protection." },
            ].map((s) => (
