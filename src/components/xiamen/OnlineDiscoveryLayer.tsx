...
export default function OnlineDiscoveryLayer() {
  ...
  return (
    <div className="space-y-0 px-2 md:px-4">
      {/* ── How to Find Xiamen Suppliers on 1688 + YouTube ── */}
      <div className="py-6">
        <h4 className="text-xl md:text-2xl font-semibold text-wf-text mb-4">How to find Xiamen suppliers on 1688</h4>
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left: Steps */}
          <div className="space-y-4">
            {[
              { step: "1", title: "Search in Chinese", desc: "Use Chinese product names. Examples: 厦门太阳镜 (sunglasses), 厦门跑步机 (treadmills), 厦门石材 (stone), 厦门LED灯 (LED lighting). WorldFirst's AI Sourcing Agent translates for you." },
              { step: "2", title: "Filter by Xiamen", desc: "Set the supplier location filter to 厦门 (Xiamen) or 福建 (Fujian) province. For fitness equipment, filter specifically to 同安 (Tong'an). For eyewear, filter to 海沧 (Haicang)." },
              { step: "3", title: "Check factory badges", desc: "Look for 实力商家 (verified merchant) and 源头工厂 (source factory) tags. Xiamen has many trading companies — check 经营范围 for 生产 (production) vs 贸易 (trade) to distinguish factories from middlemen." },
              { step: "4", title: "Verify before paying", desc: "Cross-check the business registration at gsxt.gov.cn. Tong'an and Haicang addresses = factories. Siming addresses = trading companies. Pay through 1688 World Pay for protection." },
            ].map((s) => (
