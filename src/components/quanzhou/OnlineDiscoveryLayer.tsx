...
export default function OnlineDiscoveryLayer() {
  ...
  return (
    <div className="space-y-0 px-2 md:px-4">
      {/* ── How to Find QZ Suppliers on 1688 + YouTube ── */}
      <div className="py-6">
        <h4 className="text-xl md:text-2xl font-semibold text-wf-text mb-4">How to find Quanzhou suppliers on 1688</h4>
        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Left: Steps */}
          <div className="space-y-4">
            {[
              { step: "1", title: "Search in Chinese", desc: "Use Chinese product names. Examples: 晋江运动鞋 (Jinjiang sports shoes), 石狮男装批发 (Shishi menswear wholesale), 南安石材 (Nan’an stone), 德化陶瓷 (Dehua ceramics). WorldFirst’s AI Sourcing Agent translates for you." },
              { step: "2", title: "Filter by Quanzhou", desc: "Set the supplier location filter to 泉州 (Quanzhou) or 福建 (Fujian) province. For footwear specifically, filter to 晋江 (Jinjiang) to find source factories in Chendai Town." },
              { step: "3", title: "Check factory badges", desc: "Look for 实力商家 (verified merchant) and 源头工厂 (source factory) tags. For shoes, check if they list production line capacity and MOQ per style." },
              { step: "4", title: "Verify before paying", desc: "Cross-check the business registration. Jinjiang/Chendai addresses = shoe factories. Shishi addresses = garment factories. Pay through 1688 World Pay for protection." },
            ].map((s) => (
